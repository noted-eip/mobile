...
import 'package:noted_mobile/pages/notes/editor/_editor_text_style.dart';
import 'package:openapi/openapi.dart';
import 'package:super_editor/super_editor.dart';

class NotedNoteUtility {
  static void updateDocument({
    required V1Note updatedNoteResponse,
    required DocumentComposer composer,
    required MutableDocument doc,
  }) {
    for (var i = 0; i < doc.nodes.length; i++) {
      DocumentNode oldNode = doc.nodes[i];
      ...
      DocumentNode newNode =
          NotedNoteUtility.getNodeFromBlock(updatedNoteResponse.blocks![i]);

      doc.replaceNode(
        oldNode: oldNode,
        newNode: newNode,
      );
      if (composer.selection!.extent.nodeId == oldNode.id) {
        composer.selection = DocumentSelection.collapsed(
          position: DocumentPosition(
            nodeId: newNode.id,
            nodePosition: composer.selection!.extent.nodePosition,
          ),
        );
      }
    }
  ...
  static V1Note getV1NoteFromDoc(Document doc, V1Note note) {
    List<V1Block>? blocks = [];
  ...
  static V1Block getBlockFromNode(DocumentNode node) {
    String? content;
    V1BlockType? type;
  ...
  static ListBuilder<BlockTextStyle>? getStylesFromNode(DocumentNode node) {
  ...
  static TextStylePositionBuilder? getPosFromStartAndEnd(int start, int end) {
  ...
  static TextStyleStyle? getStyleFromAttribution(Attribution attribution) {
  ...
  static String getBlockTextFromV1Block(V1Block block) {
    switch (block.type) {
  ...
  static Attribution getAttributionFromStyle(BlockTextStyle style) {
    switch (style.style) {
  ...
  static AttributedSpans? getAttributedSpanFromBlock(V1Block block) {
  ...
  static DocumentNode getNodeFromBlock(V1Block block) {
  ...
  static DocumentNode getNodeFromBlockReadOnly(V1Block block) {
    String text = getBlockTextFromV1Block(block);
  ...
  static MutableDocument createInitialDocument(
  ...
  static Map<String, List<NotedEditorTextStyle>> getAttributionFromDoc(
      MutableDocument doc) {
